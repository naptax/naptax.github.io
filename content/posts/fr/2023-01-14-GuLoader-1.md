...
Ce papier est inspiré des travaux de l'excellent Sergei Frankoff aka [@herrcore](https://twitter.com/herrcore). De toutes les personnes que je suis en Reverse Engineering, c'est bien lui le plus inspirant en analyse statique. Suivre Herrcore sur le statique et l'incroyable [@mrexodia](https://github.com/mrexodia) sur l'analyse dynamique, c'est apprendre des meilleurs.     
...
Si nous avions continué en analyse statique pure, nous aurions pu constater (avec un peu de ruse et surtout du temps d'analyse) que les auteurs de ce GuLoader (MD5:) utilisent toujours la même méthode pour passer les paramètres _key, keyLen, buffer et bufferLen_ à notre fonction de calcul de XOR:

1. Récupèration de la ```Return Address``` préalablement sauvegardée sur le haut de la pile par le CALL appelant
2. Passage en argument de cette Return Address à la fonction de Hash en tant que Début de **la clé** de Hash

Puis, le mic-mac des XOR juste pour nous masquer **la longueur de la clé (0x36)**:
...
3. L'adresse du Buffer est poussé sur la pile
4. L'adresse de la taille du Buffer est aussi poussé sur la pile

<img width="900" src="/images/guloader/guloader-bn-callduXor.png">

Cela y est nous avons nos 4 paramètres pour faire notre XOR, en avant la musique
...
La valeur du buffer à décoder avec cette clée est quant à elle logée en ```[ebp+0x140]```:

``` nasm 
000139bc  ffb540010000       push    dword [ebp+0x140]  
```
C'est bon, il ne nous reste plus qu'à passer en dynamique pour connaitre les valeurs dans la pile en breakant à 0x139ca
...
Ce qui va nous intéresser de tracer ici c'est la valeur des registres et de la stack après chaque appel de la fonction (```sub_0x12fc1``` , que nous avons pour plus confort ;-) renommé ```Calcul_XOR```. 
