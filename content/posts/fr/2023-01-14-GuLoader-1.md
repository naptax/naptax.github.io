...
Pour produire ce type de dump mémoire, à ce jour (Version 3.3.39) il n'y a rien dans la GUI du Debugger de Binary Ninja. Il faut savoir que l'introduction d'un debugger dans BINARY NINJA est somme toute assez récente et que son concepteur (@) a fait un choix qui me parait fort judicieux: Intégrer dans la GUI 'juste' un Front-end pour des moteurs de debugger existants. A ce jour BINARY NINJA en propose par défaut 2 sous Windows (LLDB et x64Dbg) et un sous LINUX (LLDB).
...
Pour faire ces 4 opérations basiques, je vous propose le simple programme RUST suivant (allez y en Python ou en C si vous êtes plus à l'aise ;-):
``` rust
```

Ok, maintenant exécutons tout ce petit monde sous _x64Dbg_ (** LISEZ BIEN LE DISCLAIMER AVANT DE FAIRE EXPLOSER LE SHELLCODE DANS VOS VM**)
