...
Pour faire ces 4 opérations basiques, je vous propose le simple programme **RUST** :heart: suivant (allez y en Python ou en C si vous êtes plus à l'aise ;-):
``` rust
use std::fs::File;
use std::io::Read;
use winapi::um::memoryapi::{VirtualAlloc, VirtualFree};
use winapi::um::winbase::{CopyMemory, MEM_COMMIT, PAGE_EXECUTE_READ, PAGE_READWRITE};
...
```
Dependances à ajouter à votre ```cargo.toml```:

``` bash
[dependencies]

winapi = { version = "0.3.8", features = ["windows", "extended"] }
