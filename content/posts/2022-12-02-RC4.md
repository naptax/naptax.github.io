...
Evidement un malware ne stockera pas sa clé secrète en clair dans une simple string; comme c'est ici le cas dans notre programme exemple (`u_char *key = "password"`). Un simple examen de la section String du binaire permettrait alors d'afficher les string candidates à être une clé déchiffrement (surtout quand la clé est "password" ;-) 
...
Disons pour notre exemple que nous souhaitons dissimuler password en additionant 3 aux octets de la chaine puis les pousser sur la stack.
