...
1. Une **très/trés lente**, basée sur des DIV ou IDIV (que tout compilateur moderne ne générera pas sauf à lui demander explicitement de privilégier la taille du code binaire au détriment des performances, comme l'option `-Os sur gcc`) [ici pour creuser le sujet](https://stackoverflow.com/questions/41183935/why-does-gcc-use-multiplication-by-a-strange-number-in-implementing-integer-divi)
...
Une fois l'utilisation de RC4 identifiée par ces différents signaux, il faut alors vous mettre en recherche de la génération KSA afin d'y localiser et extraire la clé magique ;-) 
