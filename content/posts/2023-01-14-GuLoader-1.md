...
Cette fonction n'a pas l'apparence de JUNK code et semble implémenter un algo bien réel : checkons quelques constantes.
...
En effet **5381** est une valeur importante dans l'algorithme de hash **DBJ2**. Cette algo de hash est très souvent utilisé pour faire du hashing dans les malwares (tout comme la valeur 256 caractérise un chiffrement RC4: cf mon article sur ce sujet). Il est simple, rapide et avec très peu de collisions.

On aurait donc cette fonction en 0x12AD5 qui implémenterait un hash DBJ2. On va donc prendre cette hypothèse et la nommer comme telle:

Regardons maintenant qui appelle cette fonction de hash, pour voir si notre hypothèse tient la route:

Mais dites donc on ne serait pas en train d'essayer de nous masquer des appels API Windows par la technique d'API Hash Calling en utilisant un Hash DBJ2 (ici sur mon blog) ??? !!
