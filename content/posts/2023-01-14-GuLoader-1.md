...
Par conséquent, devrions plutôt dire que nous allons analyser "une version" de GuLoader plutot que GuLoader. En effet chaque update modifie complètement le profil du payload.
...
- Chaque Malware analysé l'est évidement au sein d'une **VM** sous Windows 10 avec [100% FLARE VM de MANDIANT](https://github.com/mandiant/flare-vm) d'installé (```Host Based Indicators```)
...
Une fois extraient avec 7-Zip, voici les fichiers qui composent le package:

<img width="500" src="/images/guloader/guloader.1.png">

Jetons un coup d'oeil au fichier ```rudebies.par``` .... 

Regardons ce que PIEVE nous sort comme info.

**Rien**.

Chargeons alors ce binaire dans GHIDRA. La première partie intéressante est ce petit bout de code, je vous ai mis en commentaire ce que nous pouvons en tirer:

Jusqu'à l'adresse, que du JUNK Code n'ayant aucun impact sur le flux d'exécution.
La première instruction intéressante se situe à xxx:

Ok, on a donc une première boucle qui XOR l'hexa de l'adresse (stockée dans EDI par le POP EDI) jusqu'à l'adresse avec 0x  comme clé.
...
```
Si vous préférez procéder au déchiffrement du code XORé plutôt en interne dans GHIDRA, plutôt qu'en externe (ici en RUST), alors voici le code Python pour GHIDRA:



Allez on regarde ce binaire Stage 2 dans GHIDRA.

Une première lecture du code désassemblé laisse septique: Encore du JUNK :-(
Néanmoins, on identifie tout de même les fonctions suivantes qui semblent procéder au calcul d'une constante:

On reconstitue ces constantes:
