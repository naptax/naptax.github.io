...
Si vous avez suivi les différents [billets de PROLOG de ce blog]({{< relref "/posts/" >}}), alors le temps est venu de s'attaquer à un bon client pour l'analyse de Malware, et surtout des techniques d'évasion et d'anti : le loader **GULOADER**.
...
Nous allons étudier **une des très nombreuses versions de GuLoader**. En effet, il en existe de nombreuses car le groupe derrière ce dropper est très actif et scan tous les papiers concernant leur code. **A chaque nouvelle analyse un peu sérieuse d'une de leur version, ils réagissent et modifient rapidement de manière radicale leur malware**. 

Par conséquent, devrions plutôt dire que nous allons analyser "une version" de GuLoader plutot que GuLoader. En effet chaque update modifie complétement le profil du payload.
...
- Chaque Malware analysé l'est évidement au sein d'une **VM** sous Windows 10 avec [100% FLARE VM de MANDIANT](https://github.com/mandiant/flare-vm) d'installé
- Les toutes dernières versions de **FLARE VM** fonctionnent parfaitement avec Windows 10 (contrairement aux anciennes qui avaient des problèmes en W10 et W11)
...
- J'utilise l'excellent **RemNux** pour émuler les services internet de base (HTTPx, SSHD, SMTP, SMB, ...) et ainsi analyser les actions réseau des vilains canards 
...
Nous allons donc ouvrir notre Sample avec 7Zip qui [possède un handler pour exécuter les scripts NSIS](https://nsis.sourceforge.io/Nsis7z_plug-in).

Une fois extraient, voici les fichiers qui composent le package:

Jetons un coup d'oeil au fichier

On y découvre quelques appels API vaguemenet dissimulés (pas sur que les EDR ne sonnent pas juste avec ce trick ;-) dans une concaténations de chaines.
Voici leur rôle:

En synthèse ce script lit le fichier, le charge dans une zone mémoire exécutable puis l'éxécute. That's all pour cette partie

## Le Shellcode binaire

Chargeons le binaire dans IDA. La première partie intéressante est ce petit bout de code, je vous ai mis en commentaire ce que nous pouvons en tirer:

Ok, on a donc un XOR d du contenu de la zone xx à xx. 
Créons un petit bout de RUST pour décrypter ce code :
``` rust
use std::fs::File;
use std::io::prelude::*;
