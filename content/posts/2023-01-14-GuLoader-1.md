...
- Ah :-) on XOR le code avec la clé ```0x919e1e2e```
...
Si vous préférez procéder au déchiffrement du code XORé en **interne dans GHIDRA**, plutôt qu'en externe (ici en RUST), alors voici le code Python pour GHIDRA:

``` Python
xor_key = 0x919E1E2E
code_offset = 0x0000014E

start_addr = currentProgram.getMinAddress() + code_offset
end_addr = currentProgram.getMaxAddress()

program = currentProgram

for i, addr in enumerate(range(start_addr, end_addr)):
    current_byte = program.getMemory().getByte(addr)
    xor_byte = current_byte ^ xor_key.to_bytes(4, byteorder='little')[i % 4]
    program.getMemory().setByte(addr, xor_byte)

refresh()
```


Allez on continue: regardons ce binaire Stage 2 dans GHIDRA.
