...
Si vous préférez procéder au déchiffrement du code XORé en **interne dans HOPPER**, plutôt qu'en externe (ici en RUST), alors voici le code Python pour HOPPER:
...
for x in range(0,round(segLEN/4)):
...
**IMPORTANT:** Notez que pour mes scripts Python GHIDRA, je n'utilise plus l'interpréteur Python V2.x intégré Out of the Box dans GHIDRA, mais [le plugin GHIDRATON de MANDIANT](https://www.mandiant.com/resources/blog/ghidrathon-snaking-ghidra-python-3-scripting).

Avec ce plugin GHIDRA, il devient alors possible de scripter en PYTHON moderne.... **V3** :-) _NSA + MANDIANT_, c'est du costaud ;-)

Allez, on continue: regardons ce binaire Stage 2 dans HOPPER.
