...
1.  Le fonctionnement de mémoire et plus particulierement de la Stack
2.  Les registres
...
```c
int __cdecl additionne(int a, int b, int c);
int somme=additionne(20, 30,40); 
```

**Note** _: à ce stade ne faites pas attention au mot clé ```__cdecl``` qui permet ici juste de faire en sorte que le compilateur C 32-bits utilise la stack (et non pas les registres) pour passer les arguments à la fonction. Nous y reviendrons juste aprés lorsque nous verons les différentes conventions d'appel._
...
push 1e      ; 30 en decimal
push 14      ; 20 en decimal
call additionne 
```

La stack **AVANT** le call de la fonction ```additionne()``` serait alors la suivante :

![](../images/Stack-1.1.png)

Intéressons-nous maintenant à la fonction appelée : ```additionne()```.
...
```asm
push ebp        ; on sauvegarde sur la pile l'adresse de ebp 
mov ebp,esp     ; on déclare que le nouveau 
sub esp, 8
