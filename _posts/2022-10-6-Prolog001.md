...
La stack frame de la fonction ```main()``` **AU MOMENT DU CALL** de la fonction ```additionne()``` serait alors la suivante :

![](../images/Stack-1-1.png)

Avant d'appeler la fonction ```additionne```, la fonction ```main()``` sauvegarde les registres EAX, ECX et EDX, seulement si ces derniers risquent d'etre écrasés par la fonction ```additionne``` qu'il s'apprète à appeler. En l'occurrence notre fonction ```additionne``` n'utilisera que le registre EAX
